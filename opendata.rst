...
evropská směrnice INSPIRE [26] ze dne 25. dubna 2007, která vstoupila v platnost
...
národní legislativy České republiky novelou zákona č. 380/2009 Sb. [27].  
...
Formát GeoTIFF [16] je typickým a nejrozšířenějším otevřeným formátem pro
...
Formát JPEG [17] je kompresní určený k uložení rastrových souborů. Jeho výhodou
...
Formát PNG [76] byl vytvářen jako nástupce formátu JPEG kvůli softwarovým
...
formátem pro výměnu vektorových geodat [18]. Přestože je technologicky
...
OGC GML [19] jako otevřený standard je perspektivním formátem pro přenos
...
Populárním formátem se v poslední době stává formát GeoJSON [68], který je
...
formátů je standard OGC GeoPackage [39]. Tento formát umožňuje uložit libovolná
...
Simple Features for SQL [40]. Maximální velikost databázového souboru je 140 TB,
...
[46]. Popis implementace jednotlivých částí směrnice je obsažen v tzv.
implementačních pravidlech. Na publikaci vektorových a rastrových dat se
vztahuje technický průvodce [28]. 
...
OGC Web Map Service [20] je standard, pomocí kterého může klient požádat o
...
zpřístupnit podle standardu OGC WMTS [23]. Výhodou je rychlé odbavení příchozího
...
dlouhodobě používaná servery ČÚZK [24], který pro souřadnicový systém S-JTSK
...
OGC Web Feature Service [21] slouží k distribuci vektorových dat. Standard WFS
...
OGC Web Coverage Service [22] slouží k distribuci rastrových dat. Tento standard
...
Služba OGC Sensor Observation Service [72] je vhodná pro zpřístupnění měření ze
...
[25]. Tento formát je využíván i v dalších technologických standardech, jako je
například OGC OWS Context [38]. V principu jde o XML dokument, který obsahuje
...
Služba GitHub [41] je webové rozhraní k systému pro správu verzí Git, který byl
...
souboru  [42]. U jednodušších struktur může být limit až někde u 10 MB
...
Standard odkazuje na OGC Filter Encoding Specification (FES) [49], pomocí
...
OGC WCS podporuje ve své nejnovější verzi specifikace [50] časový rozsah
...
Formát OGC GeoPackage [39] je postavený na souborové databázi SQLite (viz
...
geodatové doméně klíčová  mezinárodní technická norma ISO 19115 [32]. Tuto normu
navíc vyžaduje i evropská směrnice INSPIRE ve svém nařízení komise o metadatech
[33]. Vlastní technickou implementací této normy se zabývají implementační
pravidla směrnice INSPIRE pro metadata [34]. Vlastní fyzické uložení metadat
geografické datové sady nebo služby je definováno navazující technickou normou
ISO 19139 (XML) [35]. Obecně lze říci, že je vhodné držet se metadatového
profilu České republiky [36], i když to v první fázi za vysloveně nutné
nepovažujeme. Důležité je, aby metadata byla dostupná přes rozhraní webové
služby OGC Catalog Service for Web (CSW) [37]. Zároveň doporučujeme tuto službu
